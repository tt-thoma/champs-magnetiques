...
def main():
    ...
    # Dossier sortie
    out_dir = parent_dir / 'champs_v4' / 'results' / 'anim_01_vectors'
    ...
    for n in range(nsteps):
        ...
        if n % frame_interval == 0:
            ...
            viz.plot_normalized(axes[0], step=6, arrow_scale=3.5, 
                               show_magnitude_bg=True, cmap='turbo')
            axes[0].axvline(nx//2, color='white', linestyle='--', 
            ...
            axes[1].axvline(nx//2, color='cyan', linestyle='--', 
                           linewidth=2, alpha=0.7, label='Interface')
            axes[1].legend(loc='upper right', fontsize=9)
            axes[1].set_title('Lignes de champ', fontsize=11)
            
            fig.suptitle(f'Refraction air/verre - t = {t*1e12:.1f} ps', 
