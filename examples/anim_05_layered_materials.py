...
def main():
    ...
    for x_start, x_end, eps, name in layers:
        ...
        print(f"  - {name:12s} : x=[{x_start:3d},{x_end:3d}], epsilon_r={eps:4.1f}, "
    ...
    out_dir = parent_dir / 'champs_v4' / 'results' / 'anim_05_multilayer'
    ...
    for n in range(nsteps):
        ...
        if n % frame_interval == 0:
            ...
            im = ax.imshow(Ez_slice.T, origin='lower', cmap='viridis', 
                          vmin=-0.3, vmax=0.3, extent=[0, nx, 0, ny])
            
            # Marquer les couches
            for x_start, x_end, eps, name in layers:
                ax.axvline(x=x_start, color='red', linestyle=':', linewidth=1, alpha=0.5)
