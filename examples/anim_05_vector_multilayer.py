...
def main():
    ...
    # Dossier sortie
    out_dir = parent_dir / 'champs_v4' / 'results' / 'anim_05_vectors'
    ...
    for n in range(nsteps):
        ...
        if n % frame_interval == 0:
            ...
            viz.plot_normalized(axes[0], step=7, arrow_scale=3.5,
                               show_magnitude_bg=True, cmap='jet')
            # Interfaces verticales
            axes[0].axvline(x2, color='white', linestyle='--', 
                           linewidth=1.5, alpha=0.6)
            axes[0].axvline(x3, color='white', linestyle='--',
                           linewidth=1.5, alpha=0.6)
            axes[0].axvline(x4, color='white', linestyle='--',
                           linewidth=1.5, alpha=0.6)
            # Labels couches
            axes[0].text(x1 + layer_thickness//2, ny - 8, 'AIR',
                        color='white', fontsize=9, ha='center',
                        bbox=dict(boxstyle='round', facecolor='black', alpha=0.7))
            axes[0].text(x2 + layer_thickness//2, ny - 8, 'VERRE',
                        color='cyan', fontsize=9, ha='center',
                        bbox=dict(boxstyle='round', facecolor='black', alpha=0.7))
            axes[0].text(x3 + layer_thickness//2, ny - 8, 'PLASTIQUE',
            ...
            axes[1].axvline(x2, color='cyan', linestyle='--',
                           linewidth=1.5, alpha=0.6)
            axes[1].axvline(x3, color='yellow', linestyle='--',
                           linewidth=1.5, alpha=0.6)
            axes[1].axvline(x4, color='white', linestyle='--',
                           linewidth=1.5, alpha=0.6)
            axes[1].set_title('Vue hybride', fontsize=11)
            
            fig.suptitle(f'Interferences multicouches - t = {t*1e12:.1f} ps',
