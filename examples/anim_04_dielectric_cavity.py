...
def main():
    ...
    wall_thickness = 5
    # Murs verticaux
    epsilon_r[cavity_x[0]:cavity_x[0]+wall_thickness, cavity_y[0]:cavity_y[1], :] = 20.0
    epsilon_r[cavity_x[1]-wall_thickness:cavity_x[1], cavity_y[0]:cavity_y[1], :] = 20.0
    # Murs horizontaux
    epsilon_r[cavity_x[0]:cavity_x[1], cavity_y[0]:cavity_y[0]+wall_thickness, :] = 20.0
    epsilon_r[cavity_x[0]:cavity_x[1], cavity_y[1]-wall_thickness:cavity_y[1], :] = 20.0
    ...
    print(f"  - Dimensions : {cavity_x[1]-cavity_x[0]} x {cavity_y[1]-cavity_y[0]} cellules")
    ...
    out_dir = parent_dir / 'champs_v4' / 'results' / 'anim_04_cavity'
    ...
    for n in range(nsteps):
        ...
        if n % frame_interval == 0:
            ...
            im = ax.imshow(Ez_slice.T, origin='lower', cmap='coolwarm', 
                          vmin=-0.1, vmax=0.1, extent=[0, nx, 0, ny])
            rect = plt.Rectangle((cavity_x[0], cavity_y[0]), 
                                cavity_x[1]-cavity_x[0], cavity_y[1]-cavity_y[0],
                                fill=False, edgecolor='yellow', linewidth=2, label='Cavité')
            ax.add_patch(rect)
