...
def main():
    ...
    # Dossier sortie
    out_dir = results_dir / "anim_02_vectors"
    ...
    for n in range(nsteps):
        ...
        if n % frame_interval == 0:
            ...
            viz.plot_normalized(
                axes[0], step=5, arrow_scale=3.5, show_magnitude_bg=True, cmap="plasma"
            )
            # Dessiner plaque
            for i in range(thickness):
                axes[0].axvline(plate_x + i, color="yellow", linewidth=3, alpha=0.8)
            axes[0].text(
                plate_x + thickness // 2,
                ny - 10,
                "METAL",
            ...
            for i in range(thickness):
                axes[1].axvline(plate_x + i, color="yellow", linewidth=3, alpha=0.8)
            axes[1].set_title("Vue hybride", fontsize=11)

            fig.suptitle(
                f"Reflexion sur conducteur - t = {t * 1e12:.1f} ps",
                fontsize=13,
                fontweight="bold",
            )
            plt.tight_layout()
