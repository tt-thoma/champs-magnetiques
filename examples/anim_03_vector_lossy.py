...
def main():
    ...
    # Dossier sortie
    out_dir = results_dir / "anim_03_vectors"
    ...
    for n in range(nsteps):
        ...
        if n % frame_interval == 0:
            ...
            viz.plot_normalized(
                axes[0], step=7, arrow_scale=3.5, show_magnitude_bg=True, cmap="hot"
            )
            axes[0].axvline(
                transition_x,
                color="cyan",
            ...
            axes[1].axvline(
                transition_x, color="yellow", linestyle="--", linewidth=2, alpha=0.8
            )
            axes[1].set_title("Lignes de champ", fontsize=11)

            fig.suptitle(
                f"Attenuation dans milieu avec pertes - t = {t * 1e12:.1f} ps",
                fontsize=13,
                fontweight="bold",
            )
            plt.tight_layout()
