...
class World:
    ...
    def calc_E2(self) -> None:
        ...
        def calculate_E(part):
            ...
            E_vec[px, py, pz] = [0, 0, 0]  # Zero out electric field at particle's position
    ...
    def calculate_properties_in_cells(self) -> tuple:
        ...
        cell_velocity_sum = np.zeros((self.field_E.shape[0], self.field_E.shape[1], self.field_E.shape[2], 3), dtype=np.float64)
    ...
    def calculate_current_density(self):
        ...
        J = V * Q[..., np.newaxis]  # Element-wise multiplication to get contributions from each particle
        J_total = np.sum(J, axis=(3))  # Sum contributions from all particles in each cell
    ...
    def calculate_rotationnel_E_fft(self):
        ...
        rotationnel_E_fft = np.stack((rotationnel_E_fft_x, rotationnel_E_fft_y, rotationnel_E_fft_z), axis=-1)
    ...
    def create_animation(
        ...
        def update():
            ...
            if animation_type == "P":
                ...
                self.plot_fields(ax, field_type=animation_type, cell_size_reduction = cell_size_reduction,min_alpha=min_alpha, max_alpha=max_alpha)
            
            if particule_visualisation == True and animation_type in ["E", "B", "T"]:
                self.plot_fields(ax, field_type=animation_type, cell_size_reduction = cell_size_reduction,min_alpha=min_alpha, max_alpha=max_alpha)
                self.plot_particle_positions(ax)
            self.calc_next()
            ax.set_title(f"Simulation - Temps écoulé (simulation): {w.temps} s", color='white')
